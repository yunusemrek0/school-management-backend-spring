...
public class TeacherController {
    ...
    @GetMapping("/getAllStudentsByAdvisorTeacher")
    @PreAuthorize("hasAnyAuthority('Teacher')")
    public List<StudentResponse> getAllStudentsByAdvisorTeacher(HttpServletRequest servletRequest){

        return teacherService.getAllStudentsByAdvisorTeacher(servletRequest);

    }

    @PostMapping("/addLessonProgram")
    @PreAuthorize("hasAnyAuthority('Admin','Dean','ViceDean')")
    public ResponseMessage<UserResponse> chooseLesson(@RequestBody @Valid AddLessonProgramToTeacherRequest lessonProgramRequest){
        return teacherService.addLessonProgramToTeacher(lessonProgramRequest);
    }
